...
def get_index(alist, name, name2=None):

    for idx, n in enumerate(alist):
        if n.startswith(name):
            return idx
        elif name2 and n.startswith(name2):
            return idx
...
def main(infile, outfile, pn, fenqi, disease):
    ...
    pn_idx = get_index(info_list, '子项目编号','子项目分期编号')
    disease_idx = get_index(info_list, '疾病名称','英文疾病名称')
    ...
    with open(outfile, 'w') as out:
        ...
        for row in range(10, 15):
            for each in sheet.row_values(row):
                # print each
                if '结题报告中样品名称' in str(each):
                    title_row_index = row
                    break

        # if '疾病发生组织类型' in sheet.row_values(title_row_index)[1]:
        #     title_row_index = 14
        # print title_row_index
        ...
        novo_idx = get_index(title, '诺禾编号') or get_index(title, '原始样本编号')
